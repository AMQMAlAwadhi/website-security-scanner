...
<body>
    ...
    def _generate_report_header(self, scan_results: Dict[str, Any], risk_metrics: Dict[str, Any]) -> str:
        ...
        target_url = metadata.get('url') or scan_results.get('url', 'N/A')
...
<div class="vulnerability-findings">
    ...
    def _render_individual_vulnerability(self, vuln: Dict[str, Any], index: int) -> str:
        ...
        # Generate real references from OWASP and CWE if references are empty
        if not references:
            references = self._generate_references_from_standards(owasp_mapping, cwe_list)
...
<div class="verification-section">
    ...
    def _generate_http_traffic_section(self, scan_results: Dict[str, Any]) -> str:
        ...
        for i, (vuln_title, instance) in enumerate(all_instances, 1):
            url = instance.get('url') or scan_results.get('url', 'N/A')
...
document.addEventListener('DOMContentLoaded', function() {{
    ...
    def _generate_references_from_standards(self, owasp_mapping, cwe_list):
        """Generate references from OWASP and CWE standards."""
        references = []
        
        # Add OWASP reference if available
        if owasp_mapping and owasp_mapping != 'N/A':
            if 'A01' in owasp_mapping:
                references.append('https://owasp.org/Top10/A01_2021-Broken_Access_Control/')
            elif 'A02' in owasp_mapping:
                references.append('https://owasp.org/Top10/A02_2021-Cryptographic_Failures/')
            elif 'A03' in owasp_mapping:
                references.append('https://owasp.org/Top10/A03_2021-Injection/')
            elif 'A04' in owasp_mapping:
                references.append('https://owasp.org/Top10/A04_2021-Insecure_Design/')
            elif 'A05' in owasp_mapping:
                references.append('https://owasp.org/Top10/A05_2021-Security_Misconfiguration/')
            elif 'A06' in owasp_mapping:
                references.append('https://owasp.org/Top10/A06_2021-Vulnerable_and_Outdated_Components/')
            elif 'A07' in owasp_mapping:
                references.append('https://owasp.org/Top10/A07_2021-Identification_and_Authentication_Failures/')
            elif 'A08' in owasp_mapping:
                references.append('https://owasp.org/Top10/A08_2021-Software_and_Data_Integrity_Failures/')
            elif 'A09' in owasp_mapping:
                references.append('https://owasp.org/Top10/A09_2021-Security_Logging_and_Monitoring_Failures/')
            elif 'A10' in owasp_mapping:
                references.append('https://owasp.org/Top10/A10_2021-Server-Side_Request_Forgery_/')
        
        # Add CWE references if available
        for cwe in cwe_list[:2]:  # Limit to first 2 CWEs
